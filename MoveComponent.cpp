...
MoveComponent::MoveComponent(Actor* ownerP, int updateOrderP)
	: Component(ownerP, updateOrderP), forwardSpeed(0.0f), angularSpeed(0.0f), warpSpeed(0.0f)
{
...
void MoveComponent::setWarpSpeed(float warpSpeedP)
{
	warpSpeed = warpSpeedP;
}
...
void MoveComponent::update(float dt)
{
	...
	if (!Maths::nearZero(warpSpeed))
	{
		Vector2 newPosition = owner.getPosition() + owner.getForward() * warpSpeed * dt;
